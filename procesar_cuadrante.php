...
if (!is_numeric($x) || !is_numeric($y)) {
    $error = "Error: Ingrese valores numéricos válidos para X y Y.";
} else {
    $x = (float)$x;
    $y = (float)$y;
...
<body class="bg-gray-100 flex items-center justify-center min-h-screen">
  ...
  <?php if (isset($error)) : ?>
    <div class="bg-red-100 border border-red-400 text-red-700 px-6 py-4 rounded-lg shadow-lg text-center w-96">
      <p class="font-semibold"><?= $error ?></p>
      <a href="ejercicio3.php" class="inline-block mt-4 px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700">
        Volver
      </a>
    </div>
  <?php else : ?>
    <div class="bg-white p-8 rounded-xl shadow-lg w-96 text-center">
      ...
      <a href="ejercicio3.php" class="inline-block mt-4 px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">
        Volver
      </a>
    </div>
  <?php endif; ?>
